...
nmoves = Parameter("nmoves", 100, """Number of Molecular Dynamics moves to be performed during the simulation.""")
...
def setupMoves(system, random_seed, GPUS):
    ...
    moves = WeightedMoves()
    moves.add(mdmove, MDMOVE_WEIGHT)
    moves.add(ligand_intra_moves,MCMOVE_WEIGHT)
...
def run():
    ...
    if not os.path.exists(restart_file.val):
        ...
        moves = WeightedMoves()
        moves.add(move0)
        # Fix code to allow restart with multiple move types
        other_moves = []
        if len(moves) > 1:
            other_moves = moves.moves()[1:]
        for other_move in other_moves:
            moves.add(other_move)
